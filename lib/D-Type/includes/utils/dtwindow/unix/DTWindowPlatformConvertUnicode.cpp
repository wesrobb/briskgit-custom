...
const struct DT_CODEPAIR_STRUCT
{
	DT_UWORD key;
	DT_UWORD ucs;
}
...
DT_ULONG ConvertKeyToUnicode(KeySym key)
{
	DT_SLONG min = 0;
	DT_SLONG max = sizeof(ket_to_ucs) / sizeof(DT_CODEPAIR_STRUCT) - 1;
	DT_SLONG mid;

	/* First check for Latin-1 characters (1:1 mapping) */
	if ((key >= 0x0020 && key <= 0x007e) || (key >= 0x00a0 && key <= 0x00ff)) return key;

	/* Also check for directly encoded 24-bit UCS characters */
	if ((key & 0xff000000) == 0x01000000) return key & 0x00ffffff;
	...
	while (max >= min)
	{
		mid = (min + max) / 2;
		     ...
		     if (ket_to_ucs[mid].key < key) min = mid + 1;
		else if (ket_to_ucs[mid].key > key) max = mid - 1;
		else return ket_to_ucs[mid].ucs; /* found it */
	}

	return 0; /* No matching Unicode value found */
}
