...
To compile as a test application define DEF_TESTAPP and type:
  ...
  g++ -DDEF_TESTAPP DTWindow.cpp DTWindowPlatform.mm -o dtwindow -O3 -Wall -I ../../../../includes/core/ -framework CoreServices -framework Cocoa -framework OpenGL

To compile as library undefine DEF_TESTAPP and type:

  rm *.o
  rm *.a
  g++ -c DTWindow.cpp DTWindowPlatform.mm -O3 -Wall -I ../../../../includes/core/
...
CDTWindow::CDTWindow(bool flags, CDTDisplay* display) : CDTWindowPlatform(flags, display)
{
}


CDTWindow::CDTWindow(CDTDisplay* display, const DT_STREAM_DESC* sd_menu) : CDTWindowPlatform(display, sd_menu)
{
}


CDTWindow::CDTWindow(CDTDisplay* display, const DT_CHAR* title, DT_SLONG w, DT_SLONG h, DT_ULONG flags, const DT_STREAM_DESC* sd_menu) : CDTWindowPlatform(display, sd_menu)
{
	Open(title, w, h, flags);
}


CDTWindow::CDTWindow(CDTDisplay* display, const DT_CHAR* title, DT_SLONG x, DT_SLONG y, DT_SLONG w, DT_SLONG h, DT_ULONG flags, const DT_STREAM_DESC* sd_menu) : CDTWindowPlatform(display, sd_menu)
{
...
{
	return dialog_fileselect(save_flag, config_flags, selected_filter, title_bar, file_types, default_filename, max_length);
}


DT_SWORD CDTWindow::SetMenu(DT_SLONG operation, DT_SLONG command, const DT_CHAR* /*extra*/)
{
	     if (operation == DV_WINDOW_MENU_UNCHECK) modify_menu_item(command, 0, false);
	else if (operation == DV_WINDOW_MENU_CHECK) modify_menu_item(command, 0, true);
	else if (operation == DV_WINDOW_MENU_DISABLE) modify_menu_item(command, 1, false);
	else if (operation == DV_WINDOW_MENU_ENABLE) modify_menu_item(command, 1, true);
	return 1;
}


void CDTWindow::onMenuCommand(DT_SLONG command, DT_SLONG extra)
{
	//printf("onMenuCommand: command=%d\n", command);

	Event_MenuCommand(command, extra);
}
